...
void jp_n16(uint8_t *instruction, uint8_t *cpu_cycles,
            uint8_t *number_of_bytes) {
...
void jp_NZ_n16(uint8_t *instruction, uint8_t *cpu_cycles,
               uint8_t *number_of_bytes) {
  bool is_zero = read_flag('Z');
  if (!is_zero) {
    jp_n16(instruction, cpu_cycles, number_of_bytes);
  } else {
    *cpu_cycles = 3;
  }
...
void jp_Z_n16(uint8_t *instruction, uint8_t *cpu_cycles,
              uint8_t *number_of_bytes) {
  bool is_zero = read_flag('Z');
  if (is_zero) {
    jp_n16(instruction, cpu_cycles, number_of_bytes);
...
void jp_NC_n16(uint8_t *instruction, uint8_t *cpu_cycles,
               uint8_t *number_of_bytes) {
  bool is_carry = read_flag('C');
  if (!is_carry) {
    jp_n16(instruction, cpu_cycles, number_of_bytes);
...
void jp_C_n16(uint8_t *instruction, uint8_t *cpu_cycles,
              uint8_t *number_of_bytes) {
  bool is_carry = read_flag('C');
  if (is_carry) {
    jp_n16(instruction, cpu_cycles, number_of_bytes);
...
void jr_n16(uint8_t *instruction, uint8_t *cpu_cycles,
            uint8_t *number_of_bytes) {
...
void jr_NZ_n16(uint8_t *instruction, uint8_t *cpu_cycles,
               uint8_t *number_of_bytes) {
  bool is_zero = read_flag('Z');
  if (!is_zero) {
    jr_n16(instruction, cpu_cycles, number_of_bytes);
  } else {
    *cpu_cycles = 2;
  }
...
void jr_Z_n16(uint8_t *instruction, uint8_t *cpu_cycles,
              uint8_t *number_of_bytes) {
  bool is_zero = read_flag('Z');
  if (is_zero) {
    jr_n16(instruction, cpu_cycles, number_of_bytes);
...
void jr_NC_n16(uint8_t *instruction, uint8_t *cpu_cycles,
               uint8_t *number_of_bytes) {
  bool is_carry = read_flag('C');
  if (!is_carry) {
    jr_n16(instruction, cpu_cycles, number_of_bytes);
...
void jr_C_n16(uint8_t *instruction, uint8_t *cpu_cycles,
              uint8_t *number_of_bytes) {
  bool is_carry = read_flag('C');
  if (is_carry) {
    jr_n16(instruction, cpu_cycles, number_of_bytes);
